...
const process_user = <T extends HasId>(user: T): T => {
  // process the user with logic here
  return user;
};

console.log(process_user({id: 1, name: "Dave"}));

// Eg. 1
const get_users_property = <T extends HasId, K extends keyof T>(users: T[], key: K): T[K][] => {
  return users.map(user => user[key]);
};
console.log(get_users_property(users_array, "email"));
console.log(get_users_property(users_array, "username"));
